/**
 * Runs heap sort on a given data set
...
public class HeapSort extends Sort{
	...
	/**
	 * Constructor
	 * @param data set to run test on
	 */
	public HeapSort(DataSet a) {
		super(a);
		super.sortType = "heap sort";
	}
	
	/**
	 * Runs heap sort on the associated data set
	 */
	...
	private <E extends Comparable<? super E>> void heapsort(E[] A) {
	...
	//INNER CLASS
	/** Max-heap implementation - Inner class*/
	public class MaxHeap<E extends Comparable<? super E>> {
		...
		public void insert(E val) {
		  ...
		  while ((curr != 0)  &&
		    ...
		    Sort.swap(Heap, curr, parent(curr));
		    curr = parent(curr);
		} 
		  }
		...
		public E removemax() {
		  ...
		  Sort.swap(Heap, 0, --n); // Swap maximum with last value
		  swaps += 1;
		...
		public E remove(int pos) {
		  ...
		  {
		    Sort.swap(Heap, pos, --n); // Swap with last value
		    
		    // If we just swapped in a big value, push it up
		    while ((pos > 0) &&
		           (Heap[pos].compareTo(Heap[parent(pos)]) > 0)) {
		      Sort.swap(Heap, pos, parent(pos));
		      pos = parent(pos);
		}
